...
reddit_data = reddit_data.rename(
    columns={'User': 'user', 'Post': 'post', 'Label': 'label'})
...
def get_pronoun_count(text):
    pronouns = re.findall(
        r'\b(I|me|mine|myself|we|us|ours|ourselves)\b', text, re.IGNORECASE)
    return len(pronouns)


reddit_data['personal_pronoun_count'] = reddit_data['post'].apply(
    get_pronoun_count)
reddit_data['personal_pronoun_itra_ratio'] = reddit_data['personal_pronoun_count'] / \
    reddit_data['personal_pronoun_count'].sum()
...
reddit_data["intra_pronoun_to_avg_ratio"] = reddit_data["num_pronouns"] / \
    np.mean(reddit_data["num_pronouns"])
...
reddit_data["mean_tree_height"], reddit_data["max_tree_height"] = zip(
    *reddit_data['post'].apply(get_tree_height))
...
def get_strict_match(text):
    ...
    textValence, textFvec = emotion(
        text, labMT, shift=True, happsList=labMTvector)
...
reddit_data["labmt_intra_avg_valence"] = reddit_data['labmt_valence'] / \
    np.mean(reddit_data['labmt_valence'])
...
reddit_data_enriched = pd.get_dummies(
    reddit_data_enriched, columns=['sentiment'])
reddit_data_enriched['personal_pronoun_count'] = normalize(
    reddit_data['personal_pronoun_count'])
reddit_data_enriched['personal_pronoun_itra_ratio'] = normalize(
    reddit_data['personal_pronoun_itra_ratio'])
...
reddit_data_enriched['intra_pronoun_to_avg_ratio'] = normalize(
    reddit_data['intra_pronoun_to_avg_ratio'])
reddit_data_enriched['mean_tree_height'] = normalize(
    reddit_data['mean_tree_height'])
reddit_data_enriched['max_tree_height'] = normalize(
    reddit_data['max_tree_height'])
