...
if (len(sys.argv) != 5):
    print("WARNING")
    print(
        "Correct input is: script.py DATA_SIZE NUM_EPOCHS [SVM-RBF|SVM-L|RF|FFNN|CNN] [5|4|3+1] [USE_CF|NO_CF]")
...
USE_CUDA = True
NUM_WORKERS = 0
save_file = '{}_{}_{}_{}.pth'.format(
    MODEL_CHOICE, LABEL_CHOICE, NUM_EPOCHS, DATA_SIZE)
...
if (LABEL_CHOICE == "5"):
    ...
    num_to_string = {0: "Supportive", 1: "Indicator",
                     2: "Ideation", 3: "Behavior", 4: "Attempt"}
elif (LABEL_CHOICE == "4"):
    string_to_num = {"Supportive": -1, "Indicator": 0, "Ideation": 1,
                     "Behavior": 2, "Attempt": 3}
    num_to_string = {0: "Indicator",
                     1: "Ideation", 2: "Behavior", 3: "Attempt"}
...
def train(model_param, device, data_loader, criterion, optimizer, epoch, print_freq=10):
    ...
    for i, (input, target) in enumerate(data_loader):
        ...
        accuracy.update(compute_batch_accuracy(
            output, target).item(), target.size(0))
        if i % print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
...
def plot_learning_curves(train_losses, test_losses, train_accuracies, test_accuracies):
    ...
    plt.xlabel('epoch')
    plt.legend(loc="best")
    plt.savefig(os.path.join(PATH_OUTPUT, "{}_{}_{}_{}_losses.png".format(
        MODEL_CHOICE, LABEL_CHOICE, NUM_EPOCHS, DATA_SIZE)), pad_inches=0)
    plt.clf()
    plt.cla()
    plt.figure()
    plt.plot(np.arange(len(test_accuracies)), train_accuracies, label='Train')
    plt.plot(np.arange(len(test_accuracies)),
             test_accuracies, label='Validation')
    plt.ylabel('Loss')
    plt.xlabel('epoch')
    plt.legend(loc="best")
    plt.savefig(os.path.join(PATH_OUTPUT, "{}_{}_{}_{}_accuracies.png".format(
        MODEL_CHOICE, LABEL_CHOICE, NUM_EPOCHS, DATA_SIZE)), pad_inches=0)
    pass
...
def plot_confusion_matrix(results):
    ...
    confusion_matrix = metrics.confusion_matrix(
        list(out[0]), list(out[1]), normalize='true')
    cm_display = metrics.ConfusionMatrixDisplay(
        confusion_matrix=confusion_matrix, display_labels=class_names)
    cm_display.plot()
    plt.savefig(os.path.join(PATH_OUTPUT, "{}_{}_{}_{}_confusion_NORMALIZED.png".format(
        MODEL_CHOICE, LABEL_CHOICE, NUM_EPOCHS, DATA_SIZE)), pad_inches=0, dpi=199)

    confusion_matrix = metrics.confusion_matrix(
        list(out[0]), list(out[1]), normalize=None)
    cm_display = metrics.ConfusionMatrixDisplay(
        confusion_matrix=confusion_matrix, display_labels=class_names)
    cm_display.plot()
    plt.savefig(os.path.join(PATH_OUTPUT, "{}_{}_{}_{}_confusion.png".format(
        MODEL_CHOICE, LABEL_CHOICE, NUM_EPOCHS, DATA_SIZE)), pad_inches=0, dpi=199)
    pass
        ...
        if (MODEL_CHOICE == 'CNN' or MODEL_CHOICE == 'FFNN'):
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                train_loss, train_accuracy = train(
                    model_used, device, train_loader, criterion, optimizer, epoch)
                test_loss, test_accuracy, test_results = evaluate(
                    model_used, device, test_loader, criterion)
            ...
            test_loss, test_accuracy, test_results = evaluate(
                best_model_used, device, test_loader, criterion)
            ...
            fold_precision = precision(true_output, pred_output)
            fold_recall = recall(true_output, pred_output)
            fold_f_score = f_score(true_output, pred_output)
            fold_ord_error = ord_error(true_output, pred_output)

            if not os.path.exists(os.path.join(PATH_OUTPUT, "FOLD_{}/".format(fold))):
                os.makedirs(os.path.join(PATH_OUTPUT, "FOLD_{}/".format(fold)))

            actual_pred_df = pd.DataFrame(
                {'true_output': true_output, 'pred_output': pred_output})
            actual_pred_df.to_csv(os.path.join(
                PATH_OUTPUT, "FOLD_{}/actual_pred_df.txt".format(fold)), sep='\t')

            with open(os.path.join(PATH_OUTPUT, "FOLD_{}/counts.txt".format(fold)), 'w') as file:
                file.write(str(dict(Counter(true_output))))
                file.write(str(dict(Counter(pred_output))))

            with open(os.path.join(PATH_OUTPUT, "FOLD_{}/stats.txt".format(fold)), "w") as text_file:
                text_file.write("precision recall f_score ord_error\n")
            text_file.write("{} {} {} {}".format(
                fold_precision, fold_recall, fold_f_score, fold_ord_error))
            ...
            if (MODEL_CHOICE == "SVM-L"):
                ...
                exit()
            clf.fit(X, y)
            predictions = clf.predict(test_ds.get_x().reshape(
                len(test_ds.get_x()), (longest_post_len+USE_CF)*300))
            ...
            rf_model.fit(X, y)
            predictions = rf_model.predict(test_ds.get_x().reshape(
                len(test_ds.get_x()), (longest_post_len+USE_CF)*300))
            test_results = list(zip(test_ds.get_y(), predictions))
        else:
            exit()
