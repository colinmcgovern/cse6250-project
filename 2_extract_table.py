import os
import pandas as pd
results = [os.path.join(dp, f) for dp, dn, filenames in os.walk(
    "figures") for f in filenames if f == 'stats.txt']
...
table_7 = pd.DataFrame(
    columns=['Approach', 'Input', 'Graded Precision', 'Graded Recall', 'F-Score', 'OE'])
...
table_7 = table_7.sort_values(
    by=['Approach', 'Input'], ascending=[False, True])
table_8 = table_8.sort_values(
    by=['Approach', 'Input'], ascending=[False, True])
table_9 = table_9.sort_values(
    by=['Approach', 'Input'], ascending=[False, True])
